...
serve(async (req) => {
  ...
  try {
    ...
    const uniqueOrgs = new Set((commissions ?? []).map((c: any) => c.org_id).filter(Boolean));

    return new Response(JSON.stringify({ clients_count: uniqueOrgs.size }), { headers: { ...corsHeaders, "Content-Type": "application/json" } });
  } catch (err) {
    return new Response(JSON.stringify({ error: String(err), clients_count: 0 }), { status: 500, headers: { ...corsHeaders, "Content-Type": "application/json" } });
  }
});
