...
serve(async (req) => {
  ...
  try {
    ...
    if (!account) return new Response(JSON.stringify({ commissions: [] }), { headers: { ...corsHeaders, "Content-Type": "application/json" } });

    const { data: commissions } = await supabase.from("partner_commissions").select("*").eq("partner_id", account.id).order("created_at", { ascending: false });

    return new Response(JSON.stringify({ commissions: commissions ?? [] }), { headers: { ...corsHeaders, "Content-Type": "application/json" } });
  } catch (err) {
