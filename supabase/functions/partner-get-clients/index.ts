...
serve(async (req) => {
  ...
  try {
    ...
    const orgIds = [...new Set((commissions ?? []).map((c: any) => c.org_id).filter(Boolean))];
    ...
    return new Response(JSON.stringify({ error: String(err), clients: [] }), { status: 500, headers: { ...corsHeaders, "Content-Type": "application/json" } });
  }
});
