...
export function CookieBanner({ forcePanel = false, onClose }: CookieBannerProps) {
  ...
  return (
    ...
    >
      {/* Backdrop */}
      <div className="absolute inset-0 bg-background/60 backdrop-blur-sm pointer-events-auto" onClick={handleClose} />
        ...
        {!showPanel ? (
          <>
            <p className="text-sm text-muted-foreground mb-5 leading-relaxed">
              GENIE IA utilise des cookies nécessaires au fonctionnement du site. Avec ton accord, nous utilisons aussi des cookies optionnels pour mesurer l'audience et améliorer l'expérience.{" "}
            ...
            <div className="flex flex-col sm:flex-row gap-2">
              <button
                ...
                className="flex-1 py-2.5 px-4 rounded-xl gradient-primary text-primary-foreground font-semibold text-sm hover:opacity-90 transition-all shadow-glow"
              >
                Tout accepter
              </button>
              <button
                onClick={handleRejectAll}
                className="flex-1 py-2.5 px-4 rounded-xl border border-border bg-muted hover:bg-muted/80 text-foreground font-semibold text-sm transition-all"
              >
                Tout refuser
              </button>
              <button
                onClick={() => setShowPanel(true)}
                className="flex-1 py-2.5 px-4 rounded-xl border border-primary/40 text-primary font-semibold text-sm hover:bg-primary/10 transition-all flex items-center justify-center gap-1.5"
              >
          ...
          <>
            ...
            <div className="flex flex-col sm:flex-row gap-2">
              ...
              <button
                ...
                className="flex-1 py-2.5 px-4 rounded-xl border border-border bg-muted hover:bg-muted/80 text-foreground font-medium text-sm transition-all"
              >
                Retour
              </button>
            </div>
...
function CookieToggleRow({
  ...
  return (
    <div className={`flex items-start gap-3 p-3 rounded-xl border ${disabled ? "border-border/40 bg-muted/30 opacity-70" : "border-border bg-muted/20"}`}>
